...
- UI backend + frontend scrubber, with checkpoint hot-reload (polls `latest.json` mtime, plus manual `POST /reload`) so the UI can pick up new checkpoints without a restart. Now backed by `ModelInferenceEngine` (real model predictions, not heuristics).
...
- Run `phaseA_20260722_103726`: `GRUEncoderFixedVar` (hidden size 32, fixed variance baseline head), best val NLL **0.507844** (baseline delta **+0.000074** vs 0.507770 baseline). This is indistinguishable from baseline — the fixed-variance diagnostic confirms the model learned no predictive signal, and the NLL run's improvement was purely from variance inflation.
